...
void cTCPServer::Send(BYTE *buff, const int len)
{
	if (!m_isConnect) { return; }

	const timeval t = { 0, m_sleepMillis };
	...
	if (/*ret != 0 &&*/ ret != SOCKET_ERROR)
	{
		for (u_int i = 0; i < sockets.fd_count; ++i)
		{
			...
			m_sendQueue.Push(sockets.fd_array[i], buff, len);
		}
	}
	//m_sendQueue.Push(m_svrSocket, buff, len);
}
